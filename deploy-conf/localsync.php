...
if (!class_exists('LocalSync')) {
    class LocalSync extends WordPress
    {
        ...
        public function setHttpHeaders($contentType = 'PLAIN')
        {
            ...
            {
            $badCodes = array(99,100,101,102,103,104,105,106,107,108,110);
...
$this->fileSeek++;
	...
	$file_path  = $iterator->getPathname();
...
//                  $dumped = file_get_contents($iwp_backup_dir.'/'.$tmp_file, false, null, 0, 4096);
            ...
            foreach ($row as $columnName => $value) {
                ...
                if ($value === null) {
                    ...
                    if($doReplace){
                        ...
                        $value = $this->findAndReplace(array($fromHTTPS, $fromHTTP), $to , $value);
        ...
        $toURL = parse_url($to);
            $retArray = array();
            if(stripos($haystack,"insert into")!==false && stripos($haystack,$from)!==false){
...
$val=str_ireplace("**||**||-lcsync,", "\',", $val);
  ...
  //$val = $this->doUtf8($val);
   ...
   if ($this->isMultiSite()) {
    ...
    $fromHTTPS = "https://".$fromURL['host'].$urlPort.$urlPath;
    $fromHTTP = "http://".$fromURL['host'].$urlPort.$urlPath;
    $replace = $this->findAndReplace(array($fromHTTPS, $fromHTTP), $to , $val);
 ...
 public function replaceInsertQuery($query)
    {
        ...
        {
        $query = str_ireplace("INSERT INTO `".$this->table."` VALUES ", '', $query);
...
/*!40101 SET character_set_client = utf8 */;\n";
        ...
        public function createSQL()
        {
            ...
            {
            $mkDir=mkdir(dirname(__FILE__)."/ls_temp", 0777);
            if(!$mkDir)
            {
            $this->setResponse(110);
            $this->biuldResponse();
                    ...
                    if (is_file($file)) {
                            ...
                            while (!$file->eof() && !$this->checkTimeBreak()) {
                              ...
                              // $current_query = utf8_encode($current_query);
                               ...
                               $current_query = str_ireplace($this->getDBPrefix(), $this->dbprefix, $current_query);
                               if($this->is_phpdump)
                               {
